...
import { getParamError } from '../utilities/param-utils';
...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  public housingLocationId?: number;
  public error?: string;
  ...
  ngOnInit(): void {
    ...
    this.housingLocationId = parseInt(getParamError(snapshot, 'id'));
 ...
 //       .pipe(take(1))
        .subscribe(
          ([housingLocation]) => {
            if (!deepEqual(this.housingLocation, housingLocation)) {
              this.housingLocation = housingLocation;
            }
          },
          error => {
            this.error = `Error retrieving housing list: ${error}`;
